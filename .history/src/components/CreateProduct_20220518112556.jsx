...
export default function CreateProduct({ picked, setModal }) {
  ...
  const [base64img, setBase64Img] = useState(picked[0].images);

  useEffect(() => {
    if(picked.length === 1) {
      setBase64Img(picked[0].images)
    }
  ...
  const handleEdit = async (event) => {
    ...
    const res = await post(
      data,
      `products/${picked[0].uuid}?_method=PUT`,
      true
    );
    if (res.message === "Data recorded succefully") {
      myForm.current.reset();
      ...
      <div style={{ textAlign: "center" }}>
        ...
        {base64img && (
            <>
              ...
              <IconButton aria-label="delete" size="small" onClick={() => setBase64Img("")}>
