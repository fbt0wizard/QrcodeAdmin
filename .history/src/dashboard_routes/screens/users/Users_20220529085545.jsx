  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        >
          <Table size="small" sx={{ minWidth: 1050, bgcolor: "#fff", mb: 1 }}>
            <TableHead>
              <TableRow>
                <TableCell>#</TableCell>
            ...
            <TableBody>
              {!noRecord ? users.map((row) => (
                <TableRow key={row.uuid} className="__table_cell_bg">
                  ...
                  <TableCell>
                      <Status color={row.status === 1 ? '#146b00' : "#07a8e7"} content={row.status === 1 ? "Active" : "Inactive"}/>
                  </TableCell>
      ...
      {!pending && userPag.totalPage !== 1 && <PaginationComponent pagnitionData={userPag} type="user"/>}
    </React.Fragment>
  );
}
