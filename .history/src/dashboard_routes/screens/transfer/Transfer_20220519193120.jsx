...
const Transfer = () => {
    ...
    axiosGet("transfers", payload).then((res) => {
      switch(res.status) {
        case 200:
          ...
          dispatch(setTotalPage({load: Math.ceil(res.data.count / 12), type: "transfer"}));
          break;
          default:
            console.log(res);
      }
    });
  }, [refresh, transferPag.start, status]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <React.Fragment>
      ...
      <TransferFilterNav setStatus={setStatus}/>
      <Box
        component="div"
        maxWidth="lg"
        sx={{
