...
const Products = () => {
  ...
  useEffect(() => {
    ...
    dispatch(setloader({load: false, type: "product"}));
    ...
    axiosGet("products", payload, false).then((res) => {
      switch (res.data.status) {
        case 200:
          dispatch(setTotalPage({load: Math.ceil(res.data.count / 12), type: "product"}));
  ...
  }, [refetch, productPag.start]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <React.Fragment>
      ...
      <ProductFilterNav  setStatus={setStatus} setName={setName} />
      <Box
        component="div"
        maxWidth="lg"
        sx={{
      ...
      >
        ...
        >
          <Table size="small" sx={{ minWidth: 935, mb: 1 }}>
            <TableHead>
              <TableRow>
                <TableCell>#</TableCell>
                    ...
                    <TableRow key={row.uuid} className="__table_cell_bg">
                      ...
                      <TableCell>
                        <Fab
                          ...
                          size="small"
                          onClick={() => {
                            func.setStorageJson("persistProduct", row)
      ...
      {!pending && productPag.totalPage !== 1 && <PaginationComponent pagnitionData={productPag} type="product" />}
    </React.Fragment>
  );
