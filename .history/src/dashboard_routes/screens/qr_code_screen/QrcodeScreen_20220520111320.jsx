...
const QrcodeScreen = () => {
  ...
  let count = qrcodePag.start + 1
    ...
    useEffect(() => {
      document.title = "Go Healthy || QR Codes"
      if (qrcodePag.showLoader) {
      dispatch(updateQrcode([]))
      }
      dispatch(setloader({load: false, type: "qr page"}))
      ...
      axiosGet("points", payload, false).then((res) => {
        if (res.status === 200) {
          dispatch(setTotalPage({load: Math.ceil(res.data.count/20), type: "qr page"}))
  ...
  return (
    <React.Fragment>
      ...
      <PaginationComponent pagnitionData={qrcodePag} type="qr page" />
    </React.Fragment>
  )
}
